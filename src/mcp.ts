...
export function createMcpServer(user?: string): McpServer {
  const who = user ?? "agent";
  ...
  server.tool(
    ...
    async ({ path, content }) => {
      await writeFile(path, content, `[${who}] Add note: ${path}`);
  ...
  server.tool(
    ...
    async ({ path, old_text, new_text }) => {
      ...
      await writeFile(path, updated, `[${who}] Update note: ${path}`);
  ...
  server.tool(
    ...
    async ({ path }) => {
      try {
        await deleteFile(path, `[${who}] Delete note: ${path}`);
      } catch {
        return {
          content: [{ type: "text" as const, text: `Note not found: ${path}` }],
          isError: true,
