...
/**
 * Find the API key entry matching `provided`, comparing in constant time.
 * Hashing first gives fixed-length buffers (so length isn't leaked) and we
 * check every entry without short-circuiting (so which key matched isn't
 * leaked via timing).
 */
function matchApiKey(provided: string) {
  const providedHash = crypto.createHash("sha256").update(provided).digest();
  let match: { name: string; key: string } | undefined;
  for (const entry of config.apiKeys) {
    const entryHash = crypto.createHash("sha256").update(entry.key).digest();
    if (crypto.timingSafeEqual(providedHash, entryHash)) match = entry;
  }
  return match;
}

function requireApiKey(req: express.Request, res: express.Response, next: express.NextFunction) {
  if (config.apiKeys.length === 0) { next(); return; } // disabled in dev if not set
  ...
  const match = provided ? matchApiKey(provided) : undefined;
  if (!match) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  }
  res.locals.user = match.name;
  next();
}
...
export function createServer() {
  ...
  app.post(
    "/webhook/github",
    express.json({ limit: "5mb", verify: (req, _res, buf) => { (req as RawBodyRequest).rawBody = buf; } }),
    async (req, res) => {
      ...
      if (config.githubWebhookSecret) {
        ...
        // HMAC over the raw bytes GitHub signed — re-serializing the parsed
        // body would not reproduce the exact payload.
        const expected =
          "sha256=" +
          crypto
  ...
  apiRouter.post("/notes", async (req, res) => {
    try {
      ...
      await writeFile(path, content, `[${res.locals.user ?? "api"}] Add note: ${path}`);
  ...
  apiRouter.patch("/notes", async (req, res) => {
    try {
      ...
      await writeFile(path, raw.replace(old_text, new_text), `[${res.locals.user ?? "api"}] Update note: ${path}`);
  ...
  apiRouter.delete("/notes/*path", async (req, res) => {
    try {
      const notePath = Array.isArray(req.params.path) ? req.params.path.join("/") : req.params.path;
      await deleteFile(notePath, `[${res.locals.user ?? "api"}] Delete note: ${notePath}`);
  ...
  app.get("/mcp/sse", requireApiKey, async (req, res) => {
    ...
    const mcpServer = createMcpServer(res.locals.user);
